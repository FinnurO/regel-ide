...
**Forfattet innhold — håndbok/rundskriv (2026-07-25, spesifisert som en utvidelse av byggesteg 1, ikke et eget nummerert byggesteg).** Ideen under (opprinnelig notert som en uavklart fremtidig mulighet) er nå konkretisert: to reelle forbilder ble undersøkt (Helsedirektoratets *"Alkoholloven med kommentarer"* og Skatteetatens håndbøker, f.eks. Merverdiavgiftshåndboken — se `02-produktkrav.md` AK-3.3.8–3.3.11 og `03-domenemodell.md` §1.1.1 for full spesifikasjon). Dette forutsetter **kun** Rettskilde-infrastrukturen som allerede finnes i byggesteg 1 (node-tre, kryssreferanser, versjonering — nå utvidet til nodenivå, `08-byggesteg1-teknisk-design.md` §2/§2.1/§2.2) — **ikke** Tjenester/Begrep fra byggesteg 2 — og hører derfor naturlig til her, ikke som et nytt byggesteg 1,5 som forskyver nummereringen av 2–9. Rettskilde-livssyklusen (`03-domenemodell.md` §3.2, `utkast → gjeldende`) dekker fortsatt "forfattet lokalt, så vedtatt"-forløpet uten endring; det nye er selve forfatterflyten (`HandbokForfatterTjeneste`, teknisk design §2.2) og node-nivå versjonering. **Ikke implementert ennå** — spesifisert som forberedelse til en egen, fokusert implementasjonsrunde, gitt at det reelt er på størrelse med sitt eget byggesteg (ny tjeneste, ny forfatter-UI, node-versjonering) selv om det ikke er nummerert som ett.

Opprinnelig idé, nå dekket av spesifikasjonen over: et mål for regel-IDE kan være å la brukeren **forfatte** lokale retningslinjer/forskrifter i verktøyet selv — ikke bare importere dem — og publisere dem når kommunestyret har vedtatt dem.

...
**AK-3.3.9** Gitt en kommentar-seksjon, skal bruker kunne koble den til én eller flere paragrafer i en eksisterende Lov/Forskrift i biblioteket, på et valgfritt detaljnivå (kapittel/bestemmelse/ledd/bokstav — `feste_niva`) — samme kryssreferanse-mekanisme som interne lovhenvisninger (§1.3 i teknisk design), ikke en egen kobling-type.
...
**AK-3.3.11** Gitt opprettelse av en ny kommentar-seksjon, skal bruker velge **dokumenttype** (kommentar/retningslinje/instruks/håndbok) — «bindende» **utledes** av valgt type og vises som metadata, brukeren skal ikke kunne sette den fritt (kommentar=ikke bindende; retningslinje/instruks/håndbok=bindende). Bindende seksjoner krever registrert godkjenner før publisering (skrives til proveniens).
**AK-3.3.12** Gitt en publisert kommentar-seksjon, skal jurist/fagansvarlig kunne merke den manuelt som «Må revideres» med en fritekstbegrunnelse, når de selv oppdager at kilden er utdatert. **v1-forenkling** (`03-domenemodell.md` §1.1.1): dette gjøres manuelt, ikke automatisk av en påvirkningsanalyse — automatisk flagging er en senere utvidelse (byggesteg 8, utenfor MVP), ikke en forutsetning for denne handlingen.

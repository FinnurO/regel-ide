...
#### 1.1.1 Håndbok/rundskriv — kommentarer til rettskilder (spesifisert 2026-07-25, ikke implementert)

**Hvorfor egen Rettskilde, ikke en annotasjon på eksisterende noder:** to reelle referansemodeller ble undersøkt — Helsedirektoratets *"Alkoholloven med kommentarer"* (kommentar festet på paragraf-nivå, med egen intern nummerering per tema — f.eks. "1.7.1", "1.7.2" — og *"Se tidligere versjoner"* per kommentar-seksjon) og Skatteetatens håndbøker (f.eks. Merverdiavgiftshåndboken: en helt parallell, egen kapittelinndelt struktur som via "Vis regelverk" peker inn i loven, eksplisitt *"bindende for etatens saksbehandling (instruks)"*). Begge er strukturelt det samme som en Rettskilde allerede er — eget node-tre, egen versjonering, kryssreferanser til andre dokumenter — bare med `kildetype='Rundskriv'`/`doctype='doc'` i stedet for `'Lov'`/`'act'`. En lett kommentar-annotasjon på eksisterende Lov/Forskrift-noder ville ikke gitt håndboken sin egen adresserbare struktur, versjonshistorikk eller mulighet til å referere flere lovparagrafer fra samme seksjon.

**Gjenbruk, ingen skjemaendring nødvendig:**
- `kildetype='Rundskriv'`, `doctype='doc'` — begge verdier fantes allerede i enum-kommentaren over, aldri brukt før nå. Én kildetype dekker både Helsedirektoratet- og Skatteetaten-varianten (samme dokumentsjanger) — se «Dokumenttype» under for hva som faktisk skiller dem.
- Node-hierarkiet (`kapittel`/`underinndeling`/`paragraf`/`ledd`/`punkt`, §2 i teknisk design) er tilstrekkelig som det er. Helsedirektoratets ad-hoc temanummerering (1.7.1, 1.7.2 …) modelleres som nøstede `ledd`/`punkt`-noder under den kommenterte paragrafens node, med den ad-hoc-nummeret i `nummer`-feltet (fritekst, ingen hardkodet nummereringsskjema) — ikke en ny nodetype.
- Kobling til den underliggende lovparagrafen en håndbok-seksjon kommenterer: `RettskildeReferanseEntitet` (`src/RegelIde.Data/Entiteter.cs`, tabell `rettskilde_referanser` i §2 i teknisk design), akkurat som interne/eksterne Lovdata-kryssreferanser i dag. Unik-indeksen er sammensatt (`fra_node_id`, `til_rettskilde_id`, `til_eid`) — én seksjon kan derfor referere flere lovparagrafer uten skjemaendring. `gjelder_eId` kan peke på et hvilket som helst nivå (kapittel/paragraf/ledd/bokstav) — «fleksibelt festenivå», se under.

**Ny kapasitet som faktisk kreves — node-nivå versjonering:** Helsedirektoratets *"Se tidligere versjoner"* er per kommentar-seksjon, ikke bare per håndbok som helhet. Det finnes ikke i dag — `rettskilde_noder` har aldri hatt egen versjon/erstatter-kjede (kun `rettskilder` har `versjon`/`entitetsstatus`/`erstatter_id`, §0). Utvidelse: samme mønster replikert ned til node-nivå, se §2 i teknisk design for feltene. En redigering av en håndbok-seksjon oppretter en ny node-versjon (ny rad, `erstatter_node_id` peker til den forrige), akkurat som en redigering av en hel Rettskilde i dag oppretter en ny rad — **ikke** en in-place-oppdatering av eksisterende node.

**Presisert 2026-07-26 — dokumenttype/bindende er en per-kommentar egenskap, ikke per håndbok.** Et tidligere utkast av dette dokumentet foreslo `er_bindende: bool` på selve `rettskilder`-raden (hele håndboken). Gjennomgang av et faktisk komponent-handoff (Helsedirektoratet- og Skatteetaten-mønstrene bygget ut i praksis) viser at **hver enkelt kommentar-seksjon** velger sin egen dokumenttype ved opprettelse — én håndbok kan inneholde både rene tolkningskommentarer og bindende instrukser side om side. Feltet hører derfor hjemme på **kommentar-noden**, ikke på hele Rettskilden, og kan ikke være en ren kolonne på den delte `rettskilde_noder`-tabellen (Lov/Forskrift-noder ville da fått en rekke alltid-NULL håndbok-spesifikke kolonner). Dette krever en egen utvidelsestabell, arbeidstittel `handbok_kommentar_metadata`, 1:1 med `rettskilde_noder.id` for rader som faktisk er en kommentar-seksjon:

| Felt | Type | Beskrivelse |
|---|---|---|
| `node_id` | ref → `rettskilde_noder.id` | 1:1 |
| `dokumenttype` | enum | `kommentar` \| `retningslinje` \| `instruks` \| `handbok` |
| `bindende` | bool, **utledet** | Kommentar=false, de tre andre=true. Aldri satt fritt av bruker — beregnes av `dokumenttype` |
| `feste_niva` | enum | `kapittel` \| `bestemmelse` \| `ledd` \| `bokstav` — hvilket detaljnivå kommentaren er festet på (uavhengig av hvilket nivå selve kommentar-noden ligger på i håndbokens eget tre) |
| `status` | enum | `under_arbeid` \| `til_godkjenning` \| `publisert` \| `ma_revideres` — se eget avsnitt under |
| `revisjonsgrunn` | string, nullable | Utfylt når `status='ma_revideres'` |
| `publisert`, `sist_faglig_endret` | date, nullable | To **atskilte** datoer — publiseringstidspunkt og siste faglige endring er ikke nødvendigvis samme hendelse |
| `underoverskrifter[]` | liste `{nummer, tittel, anker}` | Lokal innholdsliste for lange kommentarer |
| `marginord[]` | string[] | Stikkordindeks (Skatteetaten-mønster) |
| `praksis[]` | liste | Presedensreferanser med rettskildevekt — **forutsetter Presedensregisteret (byggesteg 3), ikke bygget ennå** |

**Statusløp og `ma_revideres` — forenklet for v1 (2026-07-26):** Skatteetatens/Helsedirektoratets mønster lar `ma_revideres` settes *automatisk* av en påvirkningsanalyse når kilden eller et koblet vilkår endres. Påvirkningsanalyse er eksplisitt **byggesteg 8, utenfor MVP** (`06-veikart.md`) — å bygge håndbokmodulen mot en avhengighet som ikke finnes ennå er ikke riktig rekkefølge. **V1-forenkling:** jurist/fagansvarlig setter `status='ma_revideres'` og `revisjonsgrunn` **manuelt** når de selv oppdager at en kommentar er blitt utdatert (f.eks. ved neste lovendring). Automatisk flagging fra påvirkningsanalysen er en ren senere utvidelse av samme felt — ingen skjemaendring når byggesteg 8 en gang bygges, kun en ny skriver til det samme `status`-feltet.

**Forfatterflyt (skisse, ikke låst):** i motsetning til Lov/Forskrift (parset fra Lovdata-HTML via `LovdataKonverterer`) har en håndbok ingen importpipeline — den forfattes direkte i verktøyet av jurist/fagansvarlig. Dette krever en ny tjeneste (arbeidstittel `HandbokForfatterTjeneste`, ikke `RettskildeImportTjeneste` — den er formet helt rundt `KonverteringResultat` fra HTML-parsing) med grovt sett: opprett håndbok, opprett/rediger kapittel-node, opprett/rediger kommentar-node (oppretter ny versjon ved redigering, skriver `handbok_kommentar_metadata`), koble/fjern lovparagraf-referanse, sett/fjern manuell revisjonsmerking. Se §2 i teknisk design for tjenesteskissen.

**Redigeringsflate (skisse, ikke låst):** kommentarteksten er rik tekst med et bevisst begrenset markup-sett (avsnitt, overskrift, fet/kursiv/understreking, lenke, intern referanse) — ingen tabeller, bilder eller egne farger/fonter. Interne referanser lagres som **typede pekere** (`kind` + maskin-id), ikke URL-er, nettopp for at en fremtidig påvirkningsanalyse skal kunne følge dem og oppdage når målet endrer versjon. All HTML **må saneres til det tillatte markup-settet server­side før lagring** — klienten er ikke tillit, uansett hva klientkoden selv gjør.

...
| `er_bindende` | bool, nullable | **Nytt 2026-07-25.** `NULL` for Lov/Forskrift/Presedens (spørsmålet er ikke relevant der — bindende-het følger av selve rettskildetypen). Satt (`true`/`false`) kun på Rundskriv/Virksomhetsdokument: skiller Skatteetatens håndbøker (*"bindende for etatens saksbehandling (instruks)"*) fra f.eks. Helsedirektoratets kommentarer til alkoholloven (rådgivende/fortolkende, ikke bindende). Bevisst en enkel bool, ikke gjenbruk av `rettskildevekt` (§1.7) — det er en egen, kodeliste-styrt (`KL-RETTSKILDEVEKT`) vekting spesifikt for Presedens, ikke et generelt bindende/veiledende-flagg |
...
- `kildetype='Rundskriv'`, `doctype='doc'` — begge verdier fantes allerede i enum-kommentaren over, aldri brukt før nå. Én kildetype dekker både Helsedirektoratet- og Skatteetaten-varianten (samme dokumentsjanger), skilt av `er_bindende` (over) — ikke to kildetyper.

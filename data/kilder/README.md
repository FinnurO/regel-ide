...
| Hentet | 2026-07-23 (alkoholloven/-forskriften); 2026-07-24 (forvaltningsloven); 2026-07-31 (serveringsloven); 2026-08-22 (advokatloven) |
| Filer | `nl/nl-19890602-027.xml` → `alkoholloven-LOV-1989-06-02-27.html`; `sf/sf-20050608-0538.xml` → `alkoholforskriften-FOR-2005-06-08-538.html`; `nl/nl-19670210-000.xml` → `forvaltningsloven-LOV-1967-02-10.html` (løpenummer "000" — loven har ikke noe reelt løpenummer, samme mønster som andre eldre lover); `nl/nl-19970613-055.xml` → `serveringsloven-LOV-1997-06-13-55.html`; `nl/nl-20220512-028.xml` → `advokatloven-LOV-2022-05-12-28.html` |
| Original koding | **UTF-8** — **rettet 2026-07-29.** Tidligere antatt cp1252 (og kodet slik i `LovdataBulkHenter.cs`); denne antakelsen var FEIL, bekreftet ved en ekte live-henting via `/api/rettskilder/lovdata` som produserte klassisk "UTF-8 lest som cp1252"-mojibake ("Â§", "formÃ¥l"). Alkoholloven/-forskriften ble ved innhentingen 2026-07-23 feilkodet (mojibake) og ble den gang rettet manuelt til korrekt UTF-8 på disk — men dette var etter alt å dømme SAMME kodefeil som ble bekreftet og faktisk rettet 2026-07-29, ikke en ekstern engangsfeil slik det ble antatt her tidligere. Forvaltningsloven, hentet 2026-07-24, var allerede korrekt UTF-8 da den ble lagt inn her — hvorvidt den ble hentet via samme (da fortsatt buggy) kode er ikke kjent i ettertid, kun at filen på disk var korrekt. |

`alkoholloven-LOV-1989-06-02-27.html` = "Lov om omsetning av alkoholholdig drikk m.v. (alkoholloven)", alle 11 kapitler. `alkoholforskriften-FOR-2005-06-08-538.html` = "Forskrift om omsetning av alkoholholdig drikk mv. (alkoholforskriften)". Se `06-veikart.md` byggesteg 1 for hvorfor **hele** loven/forskriften er hentet, ikke bare kapittel 4. `forvaltningsloven-LOV-1967-02-10.html` = "Lov om behandlingsmåten i forvaltningssaker (forvaltningsloven)", hele loven — nevnt i `06-veikart.md` §3.1.1 som kandidat "infrastrukturlov" å forhåndsimportere uansett testcase. `serveringsloven-LOV-1997-06-13-55.html` = "Lov om serveringsvirksomhet (serveringsloven)", hele loven (ikke kapittelinndelt — paragrafene ligger direkte under dokumentroten, `§1`–`§29`) — hentet for fasit-runde 4 (`docs/12-fasit-handbok-leveranse.md`), som trenger §3 (Bevilling), §5 (Etablererprøve) og §6 (Krav til vandel). Opprinnelig importert live via `POST /api/rettskilder/lovdata` denne runden; lagt inn som fixture her 2026-07-31 slik at reproduksjonstesten (`RundskrivReproduksjonTests.cs`) og standard oppstartsseeding får den uten en levende nettverkstilkobling til Lovdata.
...
`advokatloven-LOV-2022-05-12-28.html` = "Lov om advokater og andre som yter rettslig bistand (advokatloven)", hele loven — hentet for kandidatsøk-og-godkjenning-runden (`docs/20-virksomhetskatalog-og-rollemodell.md` §2.6.1). Testcase for virksomhetskandidat-sveipet: § 4 første ledd har ordrett "Advokattilsynet utsteder advokatbevilling", og loven inneholder også den historiske navneformen "Tilsynsrådet for advokatvirksomhet" (forgjengeren til Advokattilsynet) — dekker begge navneformer i én ekte kilde.
